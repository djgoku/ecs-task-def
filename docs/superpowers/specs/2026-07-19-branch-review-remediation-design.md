...
Remove the Claude entry from `mise.toml` and regenerate `mise.lock`. The shared
toolchain should contain only tools required to build, test, package, or release
the project.
...
runs a basic help check, generates JSON from a repository fixture with stdout
piped through `cat`, and validates that the captured output is JSON. Selecting
only the native artifact avoids trying to execute the second cross-compiled
binary.
...
- Confirm `mise install` no longer resolves or installs Claude.
